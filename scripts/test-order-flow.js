...
async function testOrderFlow() {
  ...
  try {
    ...
    categories.forEach(cat => {
      console.log(`      - ${cat.categoryName}: ${cat._count.products} productos`);
    });
    ...
    console.log(`   ✅ Encontrados ${activeProducts.length} productos activos (mostrando 5):`);
    activeProducts.forEach(product => {
      console.log(`      - ${product.productName} (${product.category.categoryName}): $${product.price} - Stock: ${product.stock}`);
    });
    ...
    console.log(`   ${hasGmailUser ? '✅' : '❌'} GMAIL_USER: ${hasGmailUser ? 'Configurado' : 'NO CONFIGURADO'}`);
    console.log(`   ${hasGmailPass ? '✅' : '❌'} GMAIL_APP_PASSWORD: ${hasGmailPass ? 'Configurado' : 'NO CONFIGURADO'}`);
    ...
    console.log(`   ${hasWhatsAppToken ? '✅' : '❌'} WHATSAPP_ACCESS_TOKEN: ${hasWhatsAppToken ? 'Configurado' : 'NO CONFIGURADO'}`);
    console.log(`   ${hasWhatsAppPhoneId ? '✅' : '❌'} WHATSAPP_PHONE_NUMBER_ID: ${hasWhatsAppPhoneId ? 'Configurado' : 'NO CONFIGURADO'}`);
    console.log(`   ${hasWhatsAppAdmins ? '✅' : '❌'} WHATSAPP_ADMIN_NUMBERS: ${hasWhatsAppAdmins ? 'Configurado' : 'NO CONFIGURADO'}`);
    ...
    recentOrders.forEach(order => {
      console.log(`      - ${order.orderNumber}: ${order.status} - $${order.total} - ${order.contactInfo?.name || 'Sin nombre'}`);
    });
    ...
    console.log(`   ✅ Encontrados ${testimonials.length} testimonios aprobados:`);
    testimonials.forEach(testimonial => {
      console.log(`      - ${testimonial.user?.firstName || 'Usuario'} - ${testimonial.rating}⭐ - ${testimonial.product?.productName || 'Producto'}`);
    });
    ...
    users.forEach(user => {
      console.log(`      - ${user.firstName || user.email}: ${user._count.orders} órdenes, ${user._count.reviews} reviews`);
    });
